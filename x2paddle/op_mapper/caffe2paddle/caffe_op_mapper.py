...
def _adjust_parameters(node):
    ...
    for idx in squeeze_indices:
        ...
        assert len(
            d.shape
        ) == 4, 'invalid shape[%s] from caffe when adjust_parameters' % (str(
            d.shape))
...
class CaffeOpMapper():
    ...
    def Input(self, node):
        self.paddle_graph.add_layer("paddle.to_tensor",
                                    inputs={},
    ...
    def Convolution(self, node):
        ...
        if data is None:
            ...
            data.append(
                np.zeros(
                    [out_channel, node.in_shapes[0][1], kernel[0],
        ...
        self.paddle_graph.add_layer("paddle.nn.Conv2D",
    ...
    def Deconvolution(self, node):
        ...
        if len(data) == 1:
            layer_attrs["bias_attr"] = False
        self.paddle_graph.add_layer("paddle.nn.Conv2DTranspose",
                                    inputs={"input": input.name},
                                    outputs=layer_outputs,
                                    **layer_attrs)
    ...
    def Pooling(self, node):
        ...
        if global_pool:
            if kernel[0] == 0:
                kernel = [1, 1]
            if params.pool == 0:
                self.paddle_graph.add_layer("paddle.nn.AdaptiveMaxPool2D",
                                            inputs={"input": input.name},
                                            outputs=layer_outputs,
                                            output_size=kernel)
            else:
                self.paddle_graph.add_layer("paddle.nn.AdaptiveAvgPool2D",
                                            inputs={"input": input.name},
                                            outputs=layer_outputs,
                                            output_size=kernel)
        else:
            ...
            if params.pool == 0:
                self.paddle_graph.add_layer("paddle.nn.MaxPool2D",
                                            inputs={"input": input.name},
                                            outputs=layer_outputs,
                                            **layer_attrs)
            else:
                self.paddle_graph.add_layer("paddle.nn.AvgPool2D",
                                            inputs={"input": input.name},
                                            outputs=layer_outputs,
                                            **layer_attrs)
    ...
    def InnerProduct(self, node):
        ...
        if data is None:
            ...
            data = []
            data.append(
                np.zeros([node.in_shapes[0][1], params.num_output
                          ]).astype("float32").astype("float32"))
            data.append(
                np.zeros([params.num_output
                          ]).astype("float32").astype("float32"))
        else:
            data = _adjust_parameters(node)
        ...
        if node.in_shapes[0][-1] != data[0].shape[0]:
            ...
            self.paddle_graph.add_layer("paddle.nn.Linear",
    ...
    def Softmax(self, node):
        ...
        self.paddle_graph.add_layer("paddle.nn.Softmax",
                                    inputs={"input": input.name},
                                    outputs=layer_outputs,
                                    **layer_attrs)
    ...
    def ReLU(self, node):
        ...
        if params.HasField('negative_slope') and params.negative_slope != 0:
            ...
            self.paddle_graph.add_layer("paddle.nn.LeakyReLU",
                                        inputs={"input": input.name},
                                        outputs=layer_outputs,
                                        **layer_attrs)
        else:
            self.paddle_graph.add_layer("paddle.nn.ReLU",
                                        inputs={"input": input.name},
                                        outputs=layer_outputs)
    ...
    def Eltwise(self, node):
        if len(node.layer.bottom
               ) == 3 and node.layer.eltwise_param.operation == 1:
            inputs_dict = {}
        ...
        assert len(node.layer.bottom
                   ) == 2, "The count of Eltwise node\'s input is not 2."
        ...
        if mode == 0:
            ...
            inputs_dict['x'] = input0_name
            inputs_dict['y'] = input1_name
            self.paddle_graph.add_layer("paddle.max",
                                        inputs=inputs_dict,
                                        outputs=[node.layer_name])
    ...
    def BatchNorm(self, node):
        ...
        if len(node.in_shapes[0]) == 2:
            self.paddle_graph.add_layer("paddle.unsqueeze",
                                        ...
                                        outputs=[input.name],
                                        axis=[2, 3])
        self.paddle_graph.add_layer("paddle.nn.BatchNorm2D",
                                    inputs={"input": input.name},
                                    outputs=layer_outputs,
                                    **layer_attrs)
        if len(node.in_shapes[0]) == 2:
    ...
    def Scale(self, node):
        if node.data is None:
            ...
            self.params[node.layer_name + "_cparam1"] = np.squeeze(
        ...
        if axis == len(node.in_shapes[0]) - 1:
            ...
            self.paddle_graph.add_layer("paddle.multiply",
                                        inputs=inputs_dict,
                                        outputs=[node.layer_name + "_mul"])
        self.paddle_graph.add_layer("self.create_parameter",
                                    inputs={},
                                    outputs=[node.layer_name + "_cparam2"],
                                    shape=self.params[node.layer_name +
                                                      "_cparam2"].shape,
    ...
    def Crop(self, node):
        ...
        self.paddle_graph.add_layer("paddle.crop",
                                    inputs={"x": input.name},
                                    outputs=[node.layer_name],
                                    shape=crop_shape,
    ...
    def Reduction(self, node):
        ...
        # operation = MEAN
            ...
            self.paddle_graph.add_layer("paddle.mean",
                                        inputs={"x": input.name},
                                        outputs=[node.layer_name],
                                        **layer_attrs)
        self.paddle_graph.add_layer("paddle.scale",
    ...
    def DetectionOutput(self, node):
        ...
        self.paddle_graph.add_layer(kernel="custom_layer:DetectionOutput",
                                    inputs=inputs_dict,
                                    outputs=layer_outputs,
                                    **layer_attrs)
    ...
    def Normalize(self, node):
        ...
        self.paddle_graph.add_layer("custom_layer:Normalize",
                                    inputs={
                                        "x": input.name,
                                        "param": param_name
                                    },
    ...
    def PriorBox(self, node):
        ...
        self.paddle_graph.add_layer("custom_layer:PriorBox",
                                    inputs=inputs_dict,
                                    outputs=layer_outputs,
                                    **layer_attrs)
    ...
    def ReLU6(self, node):
        ...
        input = self.graph.get_input_node(node, idx=0, copy=True)
        self.paddle_graph.add_layer("paddle.nn.ReLU6",
                                    inputs={"input": input.name},
                                    outputs=layer_outputs)
    ...
    def ShuffleChannel(self, node):
        ...
        self.paddle_graph.add_layer("paddle.nn.functional.channel_shuffle",
                                    inputs={"x": input.name},
                                    outputs=[node.layer_name],
                                    groups=params.group)
    ...
    def Upsample(self, node):
        ...
        self.paddle_graph.add_layer("paddle.nn.functional.interpolate",
                                    inputs={"x": input.name},
                                    outputs=[node.layer_name],
                                    **layer_attrs)
    ...
    def Select(self, node):
        ...
        self.paddle_graph.add_layer("custom_layer:Select",
                                    inputs={"x": input.name},
                                    outputs=layer_outputs,
                                    **layer_attrs)
