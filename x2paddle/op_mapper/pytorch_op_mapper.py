...
class PyTorchOpMapper(OpMapper):
    ...
    def adaptive_avg_pool2d(self, node):
        ...
        node.fluid_code.add_note("{} = math.floor({}[3] / float({}))".format(
            node.layer_name + '__stride_w', node.layer_name + '__shape',
            output_w))
        node.fluid_code.add_note("{} = math.floor({}[2] / float({}))".format(
            node.layer_name + '__stride_h', node.layer_name + '__shape',
            output_h))
        node.fluid_code.add_note("{} = {}[3] - ({} - 1) * {}".format(
            node.layer_name + '__kernel_w', node.layer_name + '__shape',
