...
class FuncConcat(object):
    ...
    def process_attrs(self):
        ...
        if "x" in self.kwargs and isinstance(self.kwargs["x"], list):
            self.kwargs["x"] = "[{}]".format(", ".join(self.args[0]))
...
class FunBuildTensor(Mapper):
    ...
    def run(self):
        shape_str = "[{}]".format(", ".join(self.args))
        self.args.clear()
...
class FunSqrt(Mapper):
    ...
    def process_attrs(self):
        rename_key(self.kwargs, "input", "s")
