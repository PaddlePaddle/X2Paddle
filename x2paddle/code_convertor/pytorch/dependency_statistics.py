...
class DependencyStatistics(ast.NodeVisitor):
    """ 获取python文件的依赖信息。
    ...
    def visit(self, node):
        ...
        out = super(DependencyStatistics, self).visit(node)
        self.nodes.pop()
    ...
    def visit_alias(self, node):
        ...
        if dependency_info.IMPORT == "*":
            ...
            if import_file_path not in self.file_dependency:
                module_sta = DependencyStatistics(import_file_path, self.file_dependency)
                module_sta.run()
...
def run(py_file_path, file_dependency):
    statistics = DependencyStatistics(py_file_path, file_dependency)
    statistics.run()