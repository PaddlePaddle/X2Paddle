...
class CaffeOptimizer(object):
        ...
        for node_name in self.graph.topo_sort:
            ...
            if node.layer_type == 'Scale':
                ...
                if parent_node.layer_type == 'BatchNorm':
                    ...
                    self.merge_bn_scale(node, parent_node, is_delete_node)
            elif node.layer_type in ['ReLU']:
                parent_node = self.graph.get_bottom_node(node, idx=0)
                if parent_node.layer_type in [
                        'Convolution', 'Deconvolution', 'InnerProduct'
                ]:
                    is_delete_node = True if len(
                        parent_node.outputs) == 1 else False
                    merge_op_activation(node, parent_node, is_delete_node)

    def merge_bn_scale(self, node, parent_node, is_delete_node=True):
        ...
        parent_param_attr['param_attr'] = string(node.layer_name + '_scale')
        parent_param_attr['bias_attr'] = string(node.layer_name + '_offset')
        if is_delete_node:
    ...
    def merge_op_activation(self, node, parent_node):
        parent_fluid_layer = parent_node.fluid_code.layers[0]
        input = parent_fluid_layer.inputs
        parent_param_attr = parent_fluid_layer.param_attr
        parent_param_attr['act'] = string(node.layer_name.lower())
