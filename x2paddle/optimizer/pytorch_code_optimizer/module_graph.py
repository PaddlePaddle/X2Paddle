                ...
                if kernel_itemset in self.combination_itemset:
                    ...
                    if all_count < min_count:
                        ...
                        if j - 1 < 0:
                            ...
                            last_itemset = copy.deepcopy(
                                layer_combination_list[j - 1])
                else:
                    if j == prefix_ids[0]:
        ...
        def is_same(sub_layers1, sub_layers2, id1, id2):
            ...
            for i, layer_id1 in enumerate(layer_id_list1):
                ...
                if len(self.pd_graph.edges_in[layer_id1]) != len(
                    ...
                    return False
                for j, ipt_layer_id1 in enumerate(
                        self.pd_graph.edges_in[layer_id1]):
                    ipt_layer_id2 = self.pd_graph.edges_in[layer_id2][j]
                    if (ipt_layer_id1 in layer_id_list1) ^ (ipt_layer_id2
                                                            in layer_id_list2):
                        return False
                ...
                if (layer_id1 in self.pd_graph.edges_out) and (
                    ...
                    if (len(self.pd_graph.edges_out[layer_id1]) > 1 and len(self.pd_graph.edges_out[layer_id2]) == 1) or \
                       (len(self.pd_graph.edges_out[layer_id1]) == 1 and len(self.pd_graph.edges_out[layer_id2]) > 1):
                        return False
                    for j, opt_layer_id1 in enumerate(
                            self.pd_graph.edges_out[layer_id1]):
            ...
            for i, sub_layers_list in enumerate(sub_layers_list_list):
                ...
                if len(sub_layers_list) > 1:
                    ...
                    self.merge_node(sub_layers_list, attrs_table,
                                    real_module_name)
        layers, nn_param_nodes, _ = rename_layers(self.pd_graph.layers,
                                                  self.param_tree,
                                                  is_rename_module=True)
