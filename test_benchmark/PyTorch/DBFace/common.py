...
class BBox:
    ...
    def __repr__(self):
        landmark_formated = ",".join([
            str(item[:2]) for item in self.landmark
        ]) if self.landmark is not None else "empty"
...
class RandomColor(object):
    ...
    def build(self, num):
        ...
        for i in range(num):
            ...
            t = (cv2.cvtColor(t, cv2.COLOR_HSV2BGR) * 255).astype(
                np.uint8).reshape(3)
    ...
    def get_index(self, label):
        if isinstance(label, int):
            ...
            raise Exception(
                "label is not support type{}, must be str or int".format(
                    type(label)))
...
def drawbbox(image,
    ...
    border = thickness / 2
    pos = (x + 3, y - 5)
    cv2.rectangle(image, intv(x - border, y - 21, w + thickness, 21), color, -1,
                  16)
