...
def main(args):
    ...
    with open('../dataset/mt5_small/pytorch_output.pkl', 'rb') as oup:
        pytorch_output = pickle.load(oup)
    pytorch_result = list()
    pytorch_result.append(pytorch_output[0])
    benchmark_pipeline = BenchmarkPipeline(
        model_dir="pd_model/inference_model/",
        model_name='mt5_small_trace',
        use_gpu=args.use_gpu,
        enable_trt=args.enable_trt,
        cpu_threads=args.cpu_threads,
        enable_mkldnn=args.enable_mkldnn)
    benchmark_pipeline.run_benchmark(data={
        "x0": np.array(data[0]),
        "x1": np.array(data[1]),
    },
