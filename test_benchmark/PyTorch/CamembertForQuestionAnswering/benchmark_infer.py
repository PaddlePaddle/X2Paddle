...
def main(args):
    ...
    with open('../dataset/CamembertForQuestionAnswering/pytorch_output.pkl', 'rb') as oup:
        pytorch_output = pickle.load(oup)
    pytorch_result = pytorch_output["output0"]
    pytorch_result = list(pytorch_result)
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model/inference_model/",
                                           model_name='CamembertForQuestionAnswering_trace',
