...
base_path = '.'
encoder_ckpt = Path(os.path.join(base_path, '../dataset/MockingBird/encoder/saved_models/pretrained.pt'))
synthesizer_ckpt = Path(os.path.join(base_path, '../dataset/MockingBird/synthesizer/saved_models/mandarin/mandarin_200k.pt'))
gan_vocoder_ckpt = Path(os.path.join(base_path, '../dataset/MockingBird/vocoder/saved_models/pretrained/g_hifigan.pt'))
rnn_vocoder_ckpt = Path(os.path.join(base_path, '../dataset/MockingBird/vocoder/saved_models/wavernn/wavernn_pretrained.pt'))
...
def convert_encoder():
    ...
    # 进行转换
    pytorch2paddle(encoder._model,
                save_dir="./pd_encoder_model_trace",
                jit_type="trace",
                # input_examples=[torch.tensor(input_data)])
                input_examples=[torch.tensor(input_data).cuda()], disable_feedback=True)
...
def convert_vocoder():
    ...
    pytorch2paddle(gan_vocoder.generator,
                ...
                enable_code_optim=False, disable_feedback=True)
                # input_examples=[torch.tensor(input_data).cuda()])
