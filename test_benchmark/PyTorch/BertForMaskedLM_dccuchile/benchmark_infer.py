...
def main(args):
    with open('../dataset/BertForMaskedLM_dccuchile/pytorch_input.pkl', 'rb') as inp:
        data = pickle.load(inp)
    with open('../dataset/BertForMaskedLM_dccuchile/pytorch_output.pkl', 'rb') as oup:
        pytorch_output = pickle.load(oup)
    pytorch_result = pytorch_output["output0"]
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model/inference_model/",
                                           model_name='BertForMaskedLM_trace',
