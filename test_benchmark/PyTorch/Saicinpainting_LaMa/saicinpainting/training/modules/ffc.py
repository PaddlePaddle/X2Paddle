...
class FourierUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_layer = torch.nn.Conv2d(in_channels=in_channels * 2 +
                                          (2 if spectral_pos_encoding else 0),
...
class SpectralTransform(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = torch.nn.Conv2d(out_channels // 2,
                                     out_channels,
                                     kernel_size=1,
                                     groups=groups,
                                     bias=False)
    ...
    def forward(self, x):
        ...
        if self.enable_lfu:
            ...
            split_no = 2
            split_s = h // split_no
            xs = torch.cat(torch.split(x[:, :c // 4], split_s, dim=-2),
                           dim=1).contiguous()
...
class FFCResNetGenerator(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(n_downsampling):
            ...
            model += [
                FFC_BN_ACT(min(max_features, ngf * mult),
