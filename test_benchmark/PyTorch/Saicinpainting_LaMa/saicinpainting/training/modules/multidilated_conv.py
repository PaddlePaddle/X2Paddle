    ...
    def __init__(self,
        ...
        if comb_mode in ('cat_out', 'cat_both'):
            ...
            if equal_dim:
                ...
                out_dims = [
                    out_dim // 2**(i + 1) for i in range(dilation_num - 1)
                ]
                out_dims.append(out_dim - sum(out_dims))
                index = []
                starts = [0] + out_dims[:-1]
                lengths = [
                    out_dims[i] // out_dims[-1] for i in range(dilation_num)
                ]
        ...
        if comb_mode in ('cat_in', 'cat_both'):
            if equal_dim:
                ...
                in_dims = [
                    in_dim // 2**(i + 1) for i in range(dilation_num - 1)
                ]
