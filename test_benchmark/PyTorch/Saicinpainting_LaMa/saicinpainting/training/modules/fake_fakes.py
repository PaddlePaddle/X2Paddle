...
class FakeFakesGenerator:
    ...
    def _make_blend_target(self, input_images):
        ...
        is_aug = (torch.rand(batch_size,
                             device=input_images.device)[:, None, None, None] <
                  self.aug_proba).float()
    ...
    def _fill_masks_with_gradient(self, masks):
        ...
        grad -= grad_for_min.view(batch_size, -1).min(-1).values[:, None, None,
                                                                 None]
        grad /= grad.view(batch_size, -1).max(-1).values[:, None, None,
                                                         None] + 1e-6
