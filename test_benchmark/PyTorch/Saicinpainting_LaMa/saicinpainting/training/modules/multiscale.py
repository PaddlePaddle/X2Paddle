        ...
        for i in range(n_downsampling):
            ...
            model += [
                nn.ConvTranspose2d(ngf * mult,
                                   ...
                                   padding=1,
                                   output_padding=1),
                up_norm_layer(int(ngf * mult / 2)), up_activation
            ]
        self.model = nn.Sequential(*model)
...
class MultiscaleResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.heads = nn.ModuleList([
            ResNetHead(input_nc,
                       ...
                       padding_type=padding_type,
                       conv_kind=conv_kind,
                       activation=activation) for i in range(n_scales)
        ])
        tail_in_feats = ngf * (2**n_downsampling) + ngf
        self.tails = nn.ModuleList([
            ResNetTail(output_nc,
                       ngf=ngf,
                       n_downsampling=n_downsampling,
                       ...
                       add_in_proj=None if
                       (i == n_scales - 1) else tail_in_feats)
    ...
    def forward(self, ms_inputs: List[torch.Tensor], smallest_scales_num: Optional[int] = None) \
        ...
        if smallest_scales_num is None:
            assert len(self.heads) == len(ms_inputs), (len(self.heads),
                                                       len(ms_inputs),
                                                       smallest_scales_num)
            smallest_scales_num = len(self.heads)
        else:
            assert smallest_scales_num == len(ms_inputs) <= len(self.heads), (
                len(self.heads), len(ms_inputs), smallest_scales_num)
            ...
            if prev_tail_features is not None:
                if prev_tail_features.shape != cur_tail_input.shape:
                    prev_tail_features = F.interpolate(
                        ...
                        mode='bilinear',
                        align_corners=False)
                cur_tail_input = torch.cat((cur_tail_input, prev_tail_features),
                                           dim=1)

            cur_out, cur_tail_feats = self.tails[scale_i](cur_tail_input,
                                                          return_last_act=True)
        ...
        if self.out_cumulative:
            ...
            for i in range(1, len(ms_features)):
                ...
                cur_out_cum = cur_out + F.interpolate(all_outputs_cum[-1],
...
class MultiscaleDiscriminatorSimple(nn.Module):
    ...
    def forward(self, ms_inputs: List[torch.Tensor], smallest_scales_num: Optional[int] = None) \
        ...
        """
        if smallest_scales_num is None:
            assert len(self.ms_impl) == len(ms_inputs), (len(self.ms_impl),
                                                         len(ms_inputs),
                                                         smallest_scales_num)
            smallest_scales_num = len(self.heads)
        else:
        ...
        return [
            cur_discr(cur_input) for cur_discr, cur_input in zip(
    ...
    def forward(self, x: torch.Tensor) -> List:
        ...
        ms_inputs = [
            F.interpolate(x,
                          ...
                          mode='bilinear',
                          align_corners=False) for f in factors
        ]
...
class DiscriminatorMultiToSingleOutputMixin:
    ...
    def forward(self, x):
        ...
        return out_feat_tuples[0][0], [
            f for _, flist in out_feat_tuples for f in flist
        ]
...
class DiscriminatorMultiToSingleOutputStackedMixin:
    ...
    def forward(self, x):
        ...
        scaled_outs = [outs[0]] + [
            F.interpolate(cur_out,
                          size=outs[0].shape[-2:],
                          mode='bilinear',
                          align_corners=False) for cur_out in outs[1:]
        ]
        out = torch.cat(scaled_outs, dim=1)
        if self.return_feats_only_levels is not None:
            feat_lists = [
                out_feat_tuples[i][1] for i in self.return_feats_only_levels
            ]
        else:
...
class MultiscaleDiscrSingleInput(SingleToMultiScaleInputMixin,
                                 DiscriminatorMultiToSingleOutputStackedMixin,
                                 MultiscaleDiscriminatorSimple):
    pass


class MultiscaleResNetSingle(GeneratorMultiToSingleOutputMixin,
                             SingleToMultiScaleInputMixin, MultiscaleResNet):
    pass
