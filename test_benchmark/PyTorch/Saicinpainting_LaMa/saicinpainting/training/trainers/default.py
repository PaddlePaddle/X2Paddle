...
def make_constant_area_crop_batch(batch, **kwargs):
    crop_y, crop_x, crop_height, crop_width = make_constant_area_crop_params(
        img_height=batch['image'].shape[2],
        img_width=batch['image'].shape[3],
        **kwargs)
    batch['image'] = batch['image'][:, :, crop_y:crop_y + crop_height,
                                    crop_x:crop_x + crop_width]
    batch['mask'] = batch['mask'][:, :, crop_y:crop_y + crop_height,
                                  crop_x:crop_x + crop_width]
    print("crop")
    exit()
    return batch
...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        if self.training and self.rescale_size_getter is not None:
            ...
            batch['image'] = F.interpolate(batch['image'],
                                           size=cur_size,
                                           mode='bilinear',
                                           align_corners=False)
            batch['mask'] = F.interpolate(batch['mask'],
                                          size=cur_size,
                                          mode='nearest')

        if self.training and self.const_area_crop_kwargs is not None:
            batch = make_constant_area_crop_batch(batch,
                                                  **self.const_area_crop_kwargs)

        #for decaptioning
        img = batch['image']
        if 'd_image' in batch:
            masked_img = batch['d_image']
        mask = batch['mask']
        if 'd_mask' in batch:
            d_mask = batch['d_mask']
        masked_img = masked_img * (1 - d_mask)
        ...
        if self.concat_mask:
            masked_img = torch.cat([masked_img, d_mask], dim=1)
        ...
        if self.fake_fakes_proba > 1e-3:
            if self.training and torch.rand(1).item() < self.fake_fakes_proba:
                batch['fake_fakes'], batch[
                    'fake_fakes_masks'] = self.fake_fakes_gen(img, mask)
