...
def make_random_irregular_mask(shape,
    ...
    for i in range(times):
        ...
        for j in range(1 + np.random.randint(5)):
            ...
            end_x = np.clip((start_x + length * np.sin(angle)).astype(np.int32),
                            0, width)
            end_y = np.clip((start_y + length * np.cos(angle)).astype(np.int32),
                            0, height)
...
class DumbAreaMaskGenerator:
    ...
    def _random_vector(self, dimension):
        if self.is_training:
            ...
            mask_side = round(
                (random.random() *
                 (upper_limit - lower_limit) + lower_limit) * dimension)
...
class OutpaintingMaskGenerator:
    ...
    def __init__(self,
        ...
        self.probs = [
            left_padding_prob, top_padding_prob, right_padding_prob,
            bottom_padding_prob
        ]
        ...
        assert len([
            x for x in [self.min_padding_percent, self.max_padding_percent]
            if (x >= 0 and x <= 1)
        ]) == 2, f"Padding percentage should be in [0,1]"
        assert sum(
            self.probs
        ) > 0, f"At least one of the padding probs should be greater than 0 - {self.probs}"
        assert len([
            x for x in self.probs if (x >= 0) and (x <= 1)
        ]) == 4, f"At least one of padding probs is not in [0,1] - {self.probs}"
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        coords = [[(0, 0), (1, self.get_padding(size=self.img_h))],
