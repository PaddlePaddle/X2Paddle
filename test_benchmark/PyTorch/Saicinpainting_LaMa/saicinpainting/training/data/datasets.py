...
class DecaptioningTrainDataset(Dataset):
    def __init__(self, file_list, mask_generator):
        ...
        self.subtitles = []
        srts = open('/ssd3/lixin41/lama-main/LaMa_test_images/srt.txt', 'r', encoding='utf-8')
        for line in srts:
            line = line.strip()
    ...
    def add_subtitle(self, img):
        ...
        font = ImageFont.truetype(ttf, random.randint(25, 80)) #, encoding="lat1")
        ...
        text_coordinate = [int((w-text_width[0])/2), random.randint(0, h-text_width[1])]
        draw.text(text_coordinate, text[0], color, font=font)
        if len(text) > 1:
            text_coordinate[1] = text_coordinate[1] + int(random.uniform(1, 2) * text_width[1])
    ...
    def __getitem__(self, item):
        ...
        self.iter_i += 1
        return dict(image=src, mask=mask2.astype('float32'), d_image=img, d_mask=mask.astype('float32'))
...
def make_default_train_dataloader(indir, kind='default', out_size=512, mask_gen_kwargs=None, transform_variant='default',
    ...
    if kind == 'default':
        ...
        #                                 transform=transform,
        #                                 **kwargs)
        dataset = DecaptioningTrainDataset('/ssd2/lixin41/LR540pcrf15.list', mask_generator=mask_generator) 
...
def make_default_val_dataset(indir, kind='default', out_size=512, transform_variant='default', **kwargs):
    ...
    if kind == 'default':
        #dataset = InpaintingEvaluationDataset(indir, **kwargs)
         dataset = InferEvaluationDataset(mask_dir='/ssd3/lixin41/lama-main/LaMa_test_images/mask', image_dir='/ssd3/lixin41/lama-main/LaMa_test_images/frames')
    elif kind == 'our_eval':
