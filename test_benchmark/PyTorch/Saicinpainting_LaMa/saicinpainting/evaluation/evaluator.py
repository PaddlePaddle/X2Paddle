...
class InpaintingEvaluator():
    ...
    def evaluate(self, model=None):
        ...
        for score_name, score in tqdm.auto.tqdm(self.scores.items(),
                                                desc='scores'):
            score.to(self.device)
            with torch.no_grad():
                score.reset()
                for batch in tqdm.auto.tqdm(self.dataloader,
                                            desc=score_name,
                                            leave=False):
        ...
        if self.integral_func is not None:
            results[(self.integral_title,
                     'total')] = dict(mean=self.integral_func(results))
...
def lpips_fid100_f1(metrics, fid_scale=100):
    neg_lpips = 1 - metrics[('lpips',
                             'total')]['mean']  # invert, so bigger is better
    fid = metrics[('fid', 'total')]['mean']
    fid_rel = max(0, fid_scale - fid) / fid_scale
...
class InpaintingEvaluatorOnline(nn.Module):
    ...
    def __init__(self,
        ...
        for idx_bin in range(self.bins_num):
            ...
            start_percent = '{:.{n}f}'.format(start_percent, n=num_digits)
            end_percent = '{:.{n}f}'.format(end_percent, n=num_digits)
            self.interval_names.append("{0}-{1}%".format(
    ...
    def _get_bins(self, mask_batch):
        ...
        area = mask_batch.view(batch_size,
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        with torch.no_grad():
            ...
            for score_name, score in self.scores.items():
                result[score_name] = score(inpainted_batch, image_batch,
                                           mask_batch)
        return result
    ...
    def evaluation_end(self, states=None):
        ...
        for score_name, score in self.scores.items():
            ...
            total_results, group_results = score.get_value(groups=self.groups,
                                                           states=cur_states)
