...
class LPIPSScore(PairwiseScore):
    ...
    def __init__(self,
        ...
        self.score = PerceptualLoss(model=model,
                                    net=net,
...
class FIDScore(EvaluatorScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        total_distance = calculate_frechet_distance(activations_pred,
                                                    activations_target,
                                                    eps=self.eps)
...
class SegmentationAwareScore(EvaluatorScore):
    ...
    def forward(self, pred_batch, target_batch, mask):
        ...
        for cur_pred_segm, cur_target_segm, cur_mask in zip(
            ...
            cur_target_class_freq_total = np.bincount(cur_target_segm,
                                                      minlength=NUM_CLASS)[None,
                                                                           ...]
            cur_target_class_freq_mask = np.bincount(cur_target_segm[cur_mask],
                                                     minlength=NUM_CLASS)[None,
                                                                          ...]
            cur_pred_class_freq_mask = np.bincount(cur_pred_segm[cur_mask],
                                                   minlength=NUM_CLASS)[None,
                                                                        ...]
        ...
        batch_pred_class_freq_mask = np.concatenate(batch_pred_class_freq_mask,
                                                    axis=0)
...
def distribute_values_to_classes(target_class_freq_by_image_mask, values,
                                 idx2name):
    ...
    return {
        idx2name[i]: val
        for i, val in enumerate(result) if total_class_freq[i] > 0
    }
...
def get_segmentation_idx2name():
    return {
        i - 1: name
        for i, name in segm_options['classes'].set_index('Idx', drop=True)
        ['Name'].to_dict().items()
    }
...
class SegmentationAwarePairwiseScore(SegmentationAwareScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        if states is not None:
            (target_class_freq_by_image_total, target_class_freq_by_image_mask,
             pred_class_freq_by_image_mask, individual_values) = states
        else:
...
class SegmentationClassStats(SegmentationAwarePairwiseScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        if states is not None:
            (target_class_freq_by_image_total, target_class_freq_by_image_mask,
             pred_class_freq_by_image_mask, _) = states
        else:
            target_class_freq_by_image_total = self.target_class_freq_by_image_total
            target_class_freq_by_image_mask = self.target_class_freq_by_image_mask
            pred_class_freq_by_image_mask = self.pred_class_freq_by_image_mask
        ...
        for label, index in grouping.items():
            ...
            group_target_class_freq_by_image_mask = target_class_freq_by_image_mask[
                index]
            group_pred_class_freq_by_image_mask = pred_class_freq_by_image_mask[
                index]
            ...
            group_pred_class_freq_diff = (
                ...
                group_target_class_freq_by_image_mask).sum(0) / (
            ...
            cur_group_results.update({
                ...
                for i, v in enumerate(
                    group_target_class_freq_by_image_total_marginal) if v > 0
            })
            cur_group_results.update({
                f'mask_freq/{self.segm_idx2name[i]}': v
                for i, v in enumerate(
                    group_target_class_freq_by_image_mask_marginal) if v > 0
            })
            cur_group_results.update({
                f'mask_freq_diff/{self.segm_idx2name[i]}': v
...
class SegmentationAwareLPIPS(SegmentationAwarePairwiseScore):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__(*args, **kwargs)
        self.score_impl = PerceptualLoss(model=model,
                                         net=net,
                                         model_path=model_path,
                                         use_gpu=use_gpu,
                                         spatial=False).eval()
    ...
    def calc_score(self, pred_batch, target_batch, mask):
        return self.score_impl(pred_batch,
                               target_batch).flatten().detach().cpu().numpy()
...
def calculade_fid_no_img(img_i, activations_pred, activations_target, eps=1e-6):
    ...
    return calculate_frechet_distance(activations_pred,
                                      activations_target,
                                      eps=eps)
...
class SegmentationAwareFID(SegmentationAwarePairwiseScore):
    ...
    def get_value(self, groups=None, states=None):
        ...
        total_results = {
            'mean':
            calculate_frechet_distance(activations_pred,
                                       activations_target,
                                       eps=self.eps),
            'std':
            0,
        ...
        for label, index in grouping.items():
            if len(index) > 1:
                ...
                group_results[
                    label] = {
                        'mean':
                        calculate_frechet_distance(group_activations_pred,
    ...
    def distribute_fid_to_classes(self, class_freq, activations_pred,
                                  activations_target):
        real_fid = calculate_frechet_distance(activations_pred,
                                              activations_target,
                                              eps=self.eps)
        ...
        return distribute_values_to_classes(class_freq, errors,
                                            self.segm_idx2name)
