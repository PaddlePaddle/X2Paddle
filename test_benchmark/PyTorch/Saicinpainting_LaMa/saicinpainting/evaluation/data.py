...
class InferDataset(Dataset):
    def __init__(self, mask_dir, image_dir, pad_out_to_modulo=None, scale_factor=None):
    ...
    def __getitem__(self, i):
        image = load_image(os.path.join(self.image_dir, self.img_filenames[i]), mode='RGB')
        mask = load_mask(os.path.join(self.mask_dir, self.mask_filenames[i]), mode='L')
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask = load_mask(os.path.join(self.mask_dir, self.mask_filenames[i]), mode='L')
        masked_image = image * (1 - mask)
        result = dict(image=image, d_image=masked_image, mask=mask[None, ...], d_mask=mask[None, ...])
