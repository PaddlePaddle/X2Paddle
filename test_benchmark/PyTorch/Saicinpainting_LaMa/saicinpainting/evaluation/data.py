...
def load_mask(fname, mode='L', return_orig=False):
    kernel = np.ones((7, 7), np.uint8)
    mask = np.array(Image.open(fname).convert(mode))
    _, mask = cv2.threshold(mask, 120, 1.0, cv2.THRESH_BINARY)
...
def load_image(fname, mode='RGB', return_orig=False):
    ...
    out_img = img.astype('float32') / 255.0
    if return_orig:
        return out_img, img
    else:
        return out_img
...
class InferDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_filenames = sorted(os.listdir(image_dir))
    ...
    def __getitem__(self, i):
        image = load_image(os.path.join(self.image_dir, self.img_filenames[i]),
                           mode='RGB')
        mask = load_mask(os.path.join(self.mask_dir, self.mask_filenames[i]),
                         mode='L')
        result = dict(image=image, mask=mask[None, ...])
        masked_image = image  #* (1 - mask)
        if self.scale_factor is not None:
            result['image'] = scale_image(result['image'], self.scale_factor)
            result['mask'] = scale_image(result['mask'],
                                         self.scale_factor,
                                         interpolation=cv2.INTER_NEAREST)

        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['image'] = pad_img_to_modulo(result['image'],
                                                self.pad_out_to_modulo)
            result['mask'] = pad_img_to_modulo(result['mask'],
                                               self.pad_out_to_modulo)
        result['d_image'] = masked_image
        result['d_mask'] = mask[None, ...]
        return result
...
class InpaintingDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.mask_filenames = sorted(
            list(
                glob.glob(os.path.join(self.datadir, '**', '*mask*.png'),
                          recursive=True)))
        self.img_filenames = [
            fname.rsplit('_mask', 1)[0] + img_suffix
...
class OurInpaintingDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.mask_filenames = sorted(
            list(
                glob.glob(os.path.join(self.datadir, 'mask', '**',
                                       '*mask*.png'),
                          recursive=True)))
        self.img_filenames = [
            os.path.join(
                self.datadir, 'img',
                os.path.basename(fname.rsplit('-', 1)[0].rsplit('_', 1)[0]) +
                '.png') for fname in self.mask_filenames
        ]
        self.pad_out_to_modulo = pad_out_to_modulo
        self.scale_factor = scale_factor
...
class PrecomputedInpaintingResultsDataset(InpaintingDataset):
    ...
    def __init__(self,
                 datadir,
                 predictdir,
                 inpainted_suffix='_inpainted.jpg',
                 **kwargs):
    ...
    def __getitem__(self, i):
        ...
        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['inpainted'] = pad_img_to_modulo(result['inpainted'],
                                                    self.pad_out_to_modulo)
        return result
...
class InpaintingEvalOnlineDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_filenames = sorted(
            list(
                glob.glob(os.path.join(self.indir, '**', f'*{img_suffix}'),
                          recursive=True)))
        self.pad_out_to_modulo = pad_out_to_modulo
        self.scale_factor = scale_factor
    ...
    def __getitem__(self, i):
        img, raw_image = load_image(self.img_filenames[i],
                                    mode='RGB',
                                    return_orig=True)
