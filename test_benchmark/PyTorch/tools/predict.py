...
class BenchmarkPipeline:
    ...
    def run_benchmark(self, data=None, pytorch_result=None, warmup=0, repeats=1, abs_diff_threshold=1e-05, relative_diff_threshold=1e-05):
        if repeats == 0:
        ...
        if isinstance(pytorch_result, list) or isinstance(pytorch_result, tuple):
    ...
    #                     relative_diff_all = np.fabs(diff) / np.fabs(results[i])
                        relative_diff_all = max_abs_diff / np.fabs(pytorch_result[i]).max()
