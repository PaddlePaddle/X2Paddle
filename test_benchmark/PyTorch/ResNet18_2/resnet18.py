...
class Resnet18(nn.Module):
    def __init__(self, ver_dim, seg_dim, normal_dim, fcdim=256, s8dim=128, s4dim=64, s2dim=32, raw_dim=32):
        ...
        self.ActivateNormal = nn.Sequential(
            nn.BatchNorm2d(3),
            nn.LeakyReLU(0.1, True)
        )
 
        self.ActivateMask = nn.Sequential(
            nn.BatchNorm2d(2),
            nn.LeakyReLU(0.1, True)
        )
    ...
    def compute_roi(self, hmax, hmin, wmax, wmin, h, w):
        ...
        roi_box = torch.Tensor([max(0, center_x - half_roi_len), max(0, center_y - half_roi_len), min(w-1, center_x + half_roi_len),
    ...
    def forward(self, x, rois = None, feature_alignment=False):
        ...
        ret = {'coarse_seg': coarse_seg_pred, 'norm': norm_pred,
               'seg': fine_seg_pred, 'vertex': ver_pred}
