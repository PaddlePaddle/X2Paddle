...
def main(args):
    with open("../dataset/IDG_test_sim_feat_new/test_sim_feat_new_inputs.pkl", "rb") as fr:
        inputs_list = pickle.load(fr)
    with open("../dataset/IDG_test_sim_feat_new/test_sim_feat_new_outputs.pkl", "rb") as fr:
        onnx_result = pickle.load(fr)
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model_dygraph/inference_model/",
                                           model_name='test_sim_feat_new',
