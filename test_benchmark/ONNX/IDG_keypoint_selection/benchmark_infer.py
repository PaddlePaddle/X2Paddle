...
def main(args):
    with open("../dataset/IDG_keypoint_selection/keypoint_selection_inputs.pkl", "rb") as fr:
        inputs_list = pickle.load(fr)
    with open("../dataset/IDG_keypoint_selection/keypoint_selection_outputs.pkl", "rb") as fr:
        onnx_result = pickle.load(fr)
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model_dygraph/inference_model/",
                                           model_name='keypoint_selection',
