...
def main(args):
    with open("../dataset/IDG_multi_modal_sequential_model/multi_modal_sequential_model_inputs_1222.pkl", "rb") as fr:
        inputs_list = pickle.load(fr)
    with open("../dataset/IDG_multi_modal_sequential_model/multi_modal_sequential_model_outputs_1222.pkl", "rb") as fr:
        onnx_result = pickle.load(fr)
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model_dygraph/inference_model/",
                                           model_name='multi_modal_sequential_model',
                                           use_gpu=args.use_gpu,
                                           enable_trt=args.enable_trt,
                                           cpu_threads=args.cpu_threads,
                                           enable_mkldnn=args.enable_mkldnn)
    benchmark_pipeline.run_benchmark(data=inputs_list, onnx_result=onnx_result, warmup=1, repeats=1)
