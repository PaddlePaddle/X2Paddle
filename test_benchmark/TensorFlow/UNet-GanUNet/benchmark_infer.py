...
def main(args):
    ...
    tf_result = np.load('../dataset/UNet-GanUNet/result.npy')
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model_dygraph/inference_model/",
                                           model_name='UNet-gan_unet',
                                           use_gpu=args.use_gpu,
                                           enable_trt=args.enable_trt,
                                           cpu_threads=args.cpu_threads,
                                           enable_mkldnn=args.enable_mkldnn)
    benchmark_pipeline.run_benchmark(data=data, tf_result=tf_result, warmup=1, repeats=1, abs_diff_threshold=1e-04, relative_diff_threshold=1e-04)
