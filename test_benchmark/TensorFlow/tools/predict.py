        ...
        if enable_trt:
            try:
                ...
                if enable_dynamic_shape:
                    ...
                    config.enable_tuned_tensorrt_dynamic_shape("shape.pbtxt", True)
...
class BenchmarkPipeline:
    def __init__(self, model_dir, model_name, batch_size=1, use_gpu=True, enable_trt=True, cpu_threads=1, enable_mkldnn=True, enable_dynamic_shape=False):
    ...
    def run_benchmark(self, data=None, tf_result=None, warmup=0, repeats=1, abs_diff_threshold=1e-05, relative_diff_threshold=1e-05):
        if repeats == 0:
...
#                     relative_diff_all = np.fabs(diff) / np.fabs(results[i])
                    relative_diff_all = max_abs_diff / np.fabs(tf_result[i]).max()
