...
def main(args):
    data = np.load('../dataset/MTCNN-ONet/input.npy')
    print(data.shape)
    with open('../dataset/MTCNN-ONet/result.pkl', 'rb') as f1:
        tf_result = pickle.load(f1)
    benchmark_pipeline = BenchmarkPipeline(model_dir="pd_model_dygraph/inference_model/",
                                           model_name='MTCNN-ONet',
                                           use_gpu=args.use_gpu,
                                           enable_trt=args.enable_trt,
                                           cpu_threads=args.cpu_threads,
                                           enable_mkldnn=args.enable_mkldnn)
    benchmark_pipeline.run_benchmark(data=data, tf_result=tf_result, warmup=1, repeats=1)
